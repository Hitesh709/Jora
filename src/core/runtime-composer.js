...
import {ContinuousWorker} from "./continuous-worker.js";
...
export function createJoraRuntime({planner,factory,delivery,controller,modelProvider=null,config=runtimeConfig()}={}) {
  ...
  const worker=new ContinuousWorker({intervalMs:config.worker.intervalMs,maxCycles:config.worker.maxCycles,cycle:async ({cycle})=>controller.run({command:cycle.command??"Improve Jora",context:{cycle,modelGateway,benchmarkStore}})});
  return {runtime:new JoraRuntime({builder,controller,continuousWorker:worker,executionStore}),modelGateway,benchmarkStore,executionStore,providers,config};
}
