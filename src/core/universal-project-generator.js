...
function reset(){targets=[];score=0;scoreEl.textContent="0";player.x=428;running=true;last=performance.now();spawn=0;status.textContent="Playing";cancelAnimationFrame(raf);raf=requestAnimationFrame(loop)}
function end(){running=false;cancelAnimationFrame(raf);status.textContent="Game over · score "+score}
function spawnTarget(){const s=20+Math.random()*22;targets.push({x:Math.random()*(canvas.width-s),y:-s,w:s,h:s,speed:2.4+Math.random()*2.5})}
function draw(){ctx.clearRect(0,0,canvas.width,canvas.height);ctx.fillStyle="#070b14";ctx.fillRect(0,0,canvas.width,canvas.height);ctx.fillStyle="#fff";ctx.fillRect(player.x,player.y,player.w,player.h);for(const t of targets){ctx.fillStyle="#f5b642";ctx.beginPath();ctx.arc(t.x+t.w/2,t.y+t.h/2,t.w/2,0,Math.PI*2);ctx.fill()}}
function loop(now){if(!running)return;const dt=Math.min(32,now-last);last=now;if(keys.has("ArrowLeft")||keys.has("a")||keys.has("A"))player.x-=player.speed;if(keys.has("ArrowRight")||keys.has("d")||keys.has("D"))player.x+=player.speed;player.x=Math.max(0,Math.min(canvas.width-player.w,player.x));spawn+=dt;if(spawn>600){spawn=0;spawnTarget()}for(let i=targets.length-1;i>=0;i--){const t=targets[i];t.y+=t.speed*dt/16;if(hit(player,t)){end();return}if(t.y>canvas.height){targets.splice(i,1);score++;scoreEl.textContent=String(score)}}draw();raf=requestAnimationFrame(loop)}
