...
function normalizeGameIntent(command){
  ...
  const corrections=[];
  const gameVocabulary=["snake","pong","tetris","racing","racer","platformer","shooter","runner","dodge","chess","sudoku","puzzle","arcade"];
...
function inferBlueprint(command){
  ...
  const game=/\b(game|arcade|racing|racer|platformer|shooter|pong|snake|snack|snak|tetris|chess|sudoku|card game|memory game|puzzle game)\b/.test(lower);
  const rawGameType=/\b(card game|memory game|chess|sudoku|snake|snack|snak|tetris|pong|racing|racer|platformer|shooter|shooting game|space shooter|runner|dodge|puzzle game)\b/.exec(lower)?.[1]?.replace(/\s+/g,"-")||"arcade";
...
function gameHtml(bp){
  ...
  if(bp.gameType==="sudoku") return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><style>body{margin:0;background:#080b12;color:#fff;font-family:system-ui;display:grid;place-items:center;min-height:100vh;padding:16px;box-sizing:border-box}.game{width:min(620px,100%);background:#121722;border:1px solid #303846;border-radius:18px;padding:18px}.top{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:12px}.muted{color:#9ca8bb;font-size:13px}.board{display:grid;grid-template-columns:repeat(9,1fr);aspect-ratio:1;border:2px solid #fff;background:#fff}.cell{border:1px solid #566070;background:#111722;color:#fff;display:grid;place-items:center;font-size:clamp(18px,5vw,30px);font-weight:700;min-width:0;cursor:pointer}.cell.fixed{background:#202a3a}.cell.selected{outline:3px solid #7dd3fc;outline-offset:-3px}.cell.wrong{background:#4a2028}.cell:nth-child(3n){border-right:2px solid #fff}.cell:nth-child(n+19):nth-child(-n+27),.cell:nth-child(n+46):nth-child(-n+54){border-bottom:2px solid #fff}.controls{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.btn{background:#202a3a;color:#fff;border:1px solid #3b4658;border-radius:9px;padding:9px 12px;cursor:pointer}.btn.primary{background:#fff;color:#111}.status{margin-top:10px}.numbers{display:grid;grid-template-columns:repeat(9,1fr);gap:6px;margin-top:10px}.num{padding:9px 0}</style></head><body><main class="game"><div class="top"><div><h2 style="margin:0">${title}</h2><div class="muted">Sudoku · fill every row, column, and 3×3 box with 1–9.</div></div><span id="score">0 / 81</span></div><div id="board" class="board"></div><div id="numbers" class="numbers"></div><div class="controls"><button class="btn primary" id="new">New Puzzle</button><button class="btn" id="check">Check</button><button class="btn" id="solve">Show Solution</button></div><div id="status" class="muted status">Select an empty cell and enter a number.</div></main><script>const puzzle=[5,3,0,0,7,0,0,0,0,6,0,0,1,9,5,0,0,0,0,9,8,0,0,0,0,6,0,8,0,0,0,6,0,0,0,3,4,0,0,8,0,3,0,0,1,7,0,0,0,2,0,0,0,6,0,6,0,0,0,0,2,8,0,0,0,4,1,9,0,0,5,0,0,0,0,8,0,0,7,9];const solution=[5,3,4,6,7,8,9,1,2,6,7,2,1,9,5,3,4,8,1,9,8,3,4,2,5,6,7,8,5,9,7,6,1,4,2,3,4,2,6,8,5,3,7,9,1,7,1,3,9,2,4,8,5,6,9,6,1,5,3,7,2,8,4,2,8,7,4,1,9,6,3,5,3,4,5,2,8,6,1,7,9];let values=[...puzzle],selected=-1;const board=document.getElementById("board"),numbers=document.getElementById("numbers"),status=document.getElementById("status"),score=document.getElementById("score");function render(){board.innerHTML="";let filled=0;values.forEach((v,i)=>{if(v)filled++;const b=document.createElement("button");b.className="cell"+(puzzle[i]?" fixed":"")+(i===selected?" selected":"")+(v&&puzzle[i]===0&&v!==solution[i]?" wrong":"");b.textContent=v||"";b.onclick=()=>{if(!puzzle[i]){selected=i;render()}};board.appendChild(b)});score.textContent=filled+" / 81"}function choose(n){if(selected<0||puzzle[selected])return;values[selected]=n;render()}for(let n=1;n<=9;n++){const b=document.createElement("button");b.className="btn num";b.textContent=n;b.onclick=()=>choose(n);numbers.appendChild(b)}document.getElementById("new").onclick=()=>{values=[...puzzle];selected=-1;status.textContent="New puzzle ready.";render()};document.getElementById("check").onclick=()=>{const bad=values.some((v,i)=>v&&puzzle[i]===0&&v!==solution[i]);if(bad)status.textContent="Some entries are incorrect.";else if(values.every(Boolean))status.textContent="Solved! Great work.";else status.textContent="So far so good — keep going."};document.getElementById("solve").onclick=()=>{values=[...solution];selected=-1;status.textContent="Solution shown.";render()};render();</script></body></html>`;
