...
export function createPlaywrightBrowserDriver({
  ...
  return {
    async verify({cwd,command="",actions=[]}={}) {
      ...
      try {
        if(startApplication) {
          started=await startApplication({cwd});
          if(!started?.ok) {
            return {ok:false,mode:"BROWSER",actionsExecuted:[],errors:["Generated application failed to start"]};
          }
        }
        ...
        const baseUrl=String(
          started?.url||
          process.env.JORA_INTERACTION_BASE_URL||
          "http://127.0.0.1:4173"
        ).replace(/\/$/,"");
            ...
            case "pointer": {
              ...
              if(box) {
                await page.mouse.move(
                  box.x+box.width*(Number(action.xRatio)||0.5),
                  box.y+box.height*(Number(action.yRatio)||0.5)
                );
