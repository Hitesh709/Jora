import {MultiAgentCoordinationEngine} from "./multi-agent-coordination-engine.js";
...
export async function createProductionJoraRuntime({config,modelGateway}={}) {
  ...
  const multiAgentCoordination=new MultiAgentCoordinationEngine(config.multiAgentCoordination);
  ...
  const buildPipeline=new AutonomousBuildPipeline({
    projectBuilder,testRunner,evaluator,runtimeVerifier,interactionVerifier,securityCouncil,benchmarkStore
  });
