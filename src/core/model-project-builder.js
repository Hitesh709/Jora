...
export class ModelProjectBuilder {
  ...
  async build({command,specification,context={},repository=this.repository,progress:progressCallback=null}={}) {
    ...
    for(const file of parsed.files){
      ...
      progress({phase:"CODING",status:"FILE_WRITTEN",message:`Wrote ${file.path}`,file:{path:file.path,bytes:Buffer.byteLength(file.content,"utf8"),content:file.content.slice(0,20000),truncated:file.content.length>20000}});
    }
