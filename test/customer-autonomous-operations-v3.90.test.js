...
test("v3.82 incident detector opens on failed health",()=>{
 ...
 assert.equal(result.incident,true); assert.equal(result.status,"INCIDENT_OPEN"); assert.equal(result.incidentRecord.reason,"PRODUCTION_HEALTH_FAILED"); assert.equal(detector.list({tenantId:"t"}).length,1);
});
