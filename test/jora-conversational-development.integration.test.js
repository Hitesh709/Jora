...
test("Jora end-to-end conversational development flow preserves project context",()=>{
  ...
  const turn=(input)=>{
    ...
    history.push({role:"user",content:input},{role:"assistant",content:"Done."});
...
test("Jora coordinates specialist work and verifies evidence before governance",()=>{
  ...
  const plan=coordinator.plan({
    requirements:[{task:"score",capability:"frontend"},{task:"tests",capability:"testing"}],
    agents:[
  ...
  const governance=new EvolutionGovernanceEngine({
    minimumBenchmarkScore:0.8,minimumDelta:0,requireSecurity:true,requireTests:true,requireApproval:true
  });
  const assessment=governance.assess({
    baseline:null,candidate:{version:"candidate-1"},evaluation:{score:0.9},
    security:{passed:true},tests:{passed:true},approval:false
  });
